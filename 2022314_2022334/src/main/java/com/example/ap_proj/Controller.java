...
//    private Label incrementscore;
    ...
    @FXML
    private void restartgame(ActionEvent event2) throws IOException {
        stream.writeCherry(0);
        stream.writeScore(0);

        //switchToGameScene(event2);
    }
    ...
    private void switchToGameScene(ActionEvent event) throws IOException {
        ...
        if (Objects.equals(text,"Revive?")){
            ...
            if(Objects.equals(text, "RESTART")) {
                stream.writeCherry(0);
                stream.writeScore(0);
                add_objects_to_pane(root);
                //player.setlabels(root);
    ...
    private void add_objects_to_pane(Pane root) throws IOException {
        if (basemount!=null){
            root.getChildren().remove(basemount.getmountain());
        }
        if (player!=null){
            //player.set
        ...
        if (targetmount!=null){
            root.getChildren().remove(targetmount.gettarget().getrect());
            root.getChildren().remove(targetmount.getmountain());
        }
        if (cherry!=null){
            root.getChildren().remove(cherry.getCherryImage());
        }
